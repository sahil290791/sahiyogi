    ...
    for (const activityName in groupedActivities) {
      ...
      const header = (
        <div className="card-header" id={`headingOne-${idAttribute}`}>
          <div className={`${items.length === 0 ? '' : 'collapsed'} accordion-title-link trigger-event`} data-toggle="collapse" data-target={`#collapseOne-${idAttribute}`} aria-expanded="true" aria-controls="collapseOne" data-action="click" data-category="lockdown-handbook" data-label={`category-accordion-${activityName}`}>
